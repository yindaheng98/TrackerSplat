...
class Deformation(nn.Module):
    def __init__(
            self,
            D=8, W=256, input_ch=27, input_ch_time=9, skips=[],
            no_grid=False, no_ds=False, no_dr=False, no_do=True,
            bounds=1.6, kplanes_config={
                'grid_dimensions': 2,
...
class DeformNetwork(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.deformation_net = Deformation(W=net_width, D=defor_depth, input_ch=(4+3)+((4+3)*scale_rotation_pe)*2, input_ch_time=timenet_output, **kwargs)
        self.register_buffer('time_poc', torch.FloatTensor([(2**i) for i in range(timebase_pe)]))
        self.register_buffer('pos_poc', torch.FloatTensor([(2**i) for i in range(posebase_pe)]))
