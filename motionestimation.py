...
def build_motion_compensater(gaussians: GaussianModel, dataset: VideoCameraDataset, device: torch.device, batch_size: int, **kwargs) -> MotionCompensater:
...
if __name__ == "__main__":
    ...
    destination_folder = save_frame_cfg_args(frame=args.start_frame)
    with torch.no_grad():
        gaussians = prepare_gaussians(
            sh_degree=args.sh_degree, device=args.device,
            load_ply=os.path.join(destination_folder, "point_cloud", "iteration_" + str(args.iteration_init), "point_cloud.ply"))
        ...
        dataset[0].save_cameras(os.path.join(destination_folder, "cameras.json"))
