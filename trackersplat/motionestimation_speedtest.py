...
from trackersplat.motionestimator.point_tracker import DetectFixMotionFuser, build_point_track_batch_motion_estimator, DataParallelPointTrackMotionEstimator
from trackersplat.motionestimator.compensater import BaseMotionCompensater, build_motion_compensater
from trackersplat.motionestimator.incremental_trainer import IncrementalTrainingMotionEstimator, build_trainer_factory, BaseTrainingProcess
from trackersplat.motionestimator.refiner import build_compensater_with_refine
from trackersplat.motionestimation import save_cfg_args, prepare_gaussians, pipeline_choices
...
def point_track_builder(estimator: str, gaussians: GaussianModel, **kwargs):
    return build_point_track_batch_motion_estimator(estimator=estimator, fuser=DetectFixMotionFuser(gaussians.to("cuda")), device="cuda", **kwargs)


def build_pipeline(pipeline: str, gaussians: GaussianModel, dataset: VideoCameraDataset, log_path: str, device: torch.device, device_ids: List[torch.device], batch_size: int, iteration: int, configs_refining: dict, **kwargs) -> MotionCompensater:
...
if __name__ == "__main__":
    ...
    with open(log_path, "w") as f:
        pass
    batch_func, motion_compensater = build_pipeline(args.pipeline, gaussians, dataset, log_path, args.device, args.parallel_device, args.batch_size, args.iteration, configs_refining, **configs)
